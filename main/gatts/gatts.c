...
{
    ESP_LOGI(GATTS_TABLE_TAG, "prepare write, handle = %d, value len = %d, : %s", param->write.handle, param->write.len, param->write.value);
    ...
    ESP_LOGI(GATTS_TABLE_TAG, "Translating ");
    /*
     * At this point, a value has been sent by the client to the server.
     * Copy the value to another a variable that will be used for the encryption
     * prior to sending out the data through LORA
     */
    uint8_t bpm = prepare_write_env->prepare_buf;
